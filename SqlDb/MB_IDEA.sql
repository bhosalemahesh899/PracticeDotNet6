...
DECLARE @AlterNumTableQuery VARCHAR(2000);
...
WHILE (@@FETCH_STATUS = 0)
	...
	DECLARE @Counter INT 
		...
		BEGIN
			...
			SET @AlterNumTableQuery += 'ALTER TABLE ' + @NumTable + ' ADD ';
			...
			IF EXISTS (SELECT * FROM #TXT_EXISTING_COLUMNS)
				BEGIN
					...
					SELECT * FROM #TXT_EXISTING_COLUMNS WHERE COLUMN_NAME NOT IN(SELECT COLUMN_NAME FROM #NUM_EXISTING_COLUMNS);
